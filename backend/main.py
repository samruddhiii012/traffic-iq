...
JWT_SECRET_KEY = "trafficiq-super-secret-key-change-later"
JWT_ALGORITHM = "HS256"
...
@app.get("/community/reports/{report_id}/comments")
...
# =========================================================
# COMMUNITY - ADD COMMENT / TRAFFIC TALK
# =========================================================

@app.post("/community/reports/{report_id}/comments")
...
def reverse_geocode(latitude: float, longitude: float):
    """
    Convert latitude/longitude into a readable location name.
    """
...
def processed_media(file_path: str):
    
    requested_path = (PROCESSED_DIR / file_path).resolve()
    processed_root = PROCESSED_DIR.resolve()
    
    if not requested_path.is_relative_to(processed_root):
        raise HTTPException(
            status_code=400,
    ...
    extention = requested_path.suffix.lower()
...
async def upload_video(
    ...
    # EXIF case
    #
    # Location can initially be empty because
    # traffic_processor.py will try to read GPS.
    # -----------------------------------------------------
    ...
    if location_source == "EXIF GPS":
        ...
        if extension not in {
            ...
            raise HTTPException(
                status_code=400,
                detail="EXIF GPS is available only for photos."
            )
    ...
    # -----------------------------------------------------
        ...
        if not location.strip():
            raise HTTPException(
                status_code=400,
                detail="Please provide an analysis location."
            )
    ...
    if result is None:
        ...
        raise HTTPException(
            status_code=400,
            detail=("No traffic vehicles detected. " 
                    "Please upload an image or video containing. "
    ...
    if location_source == "EXIF GPS":
        ...
        if not result.get("has_exif_gps", False):
            
            raise HTTPException(
                status_code=400,
                detail=(
                    "No GPS information was found in this photo. "
                    "Please choose Enter Manually or Use Current Location."
                )
            )
    ...
    # -----------------------------------------------------
        ...
        if latitude is None or longitude is None:
            ...
            raise HTTPException(
                status_code=400,
                detail="Current loaction coordinates are missing."
            )
    ...
    relative_path = result["output_path"].relative_to(
        PROCESSED_DIR
    )
    
    output_url = ( 
        f"/processed/{relative_path.as_posix()}"
    )
    ...
    return {
        "message": "Traffic media processed successfully.",
